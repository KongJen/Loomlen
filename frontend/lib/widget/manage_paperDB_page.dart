...
class ManagePaperDBPage extends StatefulWidget {
  final String fileId;
  final String roomId;
  final PaperDBProvider paperDBProvider;
  final DrawingDBService drawingDBService;

  const ManagePaperDBPage({
    Key? key,
    required this.fileId,
    required this.roomId,
    required this.paperDBProvider,
    required this.drawingDBService,
  }) : super(key: key);

  @override
  State<ManagePaperDBPage> createState() => _ManagePaperDBPageState();
}

class _ManagePaperDBPageState extends State<ManagePaperDBPage>
  ...
  Widget build(BuildContext context) {
    final paperIds = widget.paperDBProvider.getPaperIdsByFileId(widget.fileId);
  ...
  Widget _buildPageList(BuildContext context, List<String> paperIds) {
    return ReorderableListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final paper = widget.paperDBProvider.getPaperDBById(paperId);
                            ...
                            children: [
                              ...
                              CustomPaint(
                                painter: DrawingThumbnailPainter(
                                  drawingPoints: widget.drawingDBService
  ...
  void _movePaper(int fromIndex, int toIndex) {
    widget.paperDBProvider.swapPaperOrder(widget.fileId, fromIndex, toIndex);
  }
  ...
  void _deletePaper(BuildContext context, String paperId) {
    widget.paperDBProvider.removePaper(paperId);
  ...
  Future<void> _addNewPage(BuildContext context, int position) async {
    ...
    Future.delayed(const Duration(milliseconds: 300), () {
      widget.paperDBProvider.insertPaperAt(
        widget.fileId,
        widget.roomId,
        position,
        template,
